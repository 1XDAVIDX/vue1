...
export default {
  setup() {
    ...
    const facturapedido = async ()=>{
      try {
        const respuesta = await axios.get('http://localhost:8080/compra/listar');
...
<template>
  ...
  <div class="box">
    ...
    <h2>¡Bienvenido a Mascotas ADMIN! </h2>
          ...
          <div class="cards">
            <ul>
              <li v-for="i in pedido" :key="i" class="card">
                <b class="pedidoespacios">ID Compra: {{ i.idCompra }}</b>
                <b class="pedidoespacios">ID Usuario:{{ i.idUsuario }}</b>       
                <b class="pedidoespacios">ID Producto:{{ i.idProducto }}</b>
